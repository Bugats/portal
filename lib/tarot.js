...
function buildInterpretation(card, position) {
  ...
  const safeMeaning = meaning || "neliela perspektīvas maiņa";
  const orientationLabel =
    card.orientation === "reversed" ? "apgriezta" : "taisni";
  const positionText = position ? position.toLowerCase() : "šis brīdis";
  const templates = [
    `Pozīcijā "${positionText}" kārts ${card.name} (${orientationLabel}) norāda uz: ${safeMeaning}.`,
    `${card.name} (${orientationLabel}) ${positionText} pozīcijā uzsver ${safeMeaning}.`,
    `Šajā pozīcijā ${card.name} (${orientationLabel}) stāsta par ${safeMeaning}.`,
  ];
...
export function buildSummary(cards) {
  ...
  const finalText = lastCard
    ? `Kopējais vēstījums: ${lastCard.name} norāda uz ${closingMeaning}.`
