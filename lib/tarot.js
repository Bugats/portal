...
function orientationNote(card) {
  if (card.orientation === "reversed") {
    return "Apgriezta kārts norāda uz aizkavēšanos vai iekšējiem šķēršļiem.";
  }

  return "Taisnā kārts norāda uz atvērtu, plūstošu enerģiju.";
}
...
function buildInterpretation(card, position) {
  ...
  const templates = [
    `Pozīcijā "${positionText}" kārts ${card.name} (${orientationLabel}) norāda uz ${baseMeaning}. ${orientationNote(
      card
    )}`,
    `${card.name} (${orientationLabel}) ${positionText} pozīcijā uzsver ${baseMeaning}. ${orientationNote(
      card
    )}`,
    `Šajā pozīcijā ${card.name} (${orientationLabel}) stāsta par ${baseMeaning}. ${orientationNote(
      card
    )}`,
  ];
...
export function buildSummary(cards) {
  ...
  const finalText = cards.length
    ? `Kopējais vēstījums: ${combined}.`
    : "Kopējais vēstījums: uzticies nākamajam solim.";
